...
{ 
   ...
   Double_t peak_ne = y[0];
//   Double_t amax = (0.03753509 - 0.003235)/ne;
   Double_t alpha = y[1];
   Double_t sigma = y[2];
   Double_t r0 = y[3];
...
//   Double_t t_max = t0 + amax/v0;
   ...
   if ( t>=t0 && t<=(t_max) ){
      ...
      double norm = peak_ne*(1.00/sqrt(sigma*2*TMath::Pi()));
      Double_t ne = peak_ne*exp(-0.5*(t - t_peak)*(t - t_peak)/(sigma*sigma) );

      Double_t sqrt_arg = 2*r*v0*dt - (v0*dt)*(v0*dt);
...
//         par[1] = 0.0000001; // amax
        ...
        line->SetParName(0,"ne");
         ...
         line->SetParName(2,"#sigma_{gauss}");
         line->SetParName(3,"Radius of the blob [pc]");
...
void fit_dm_variations_gauss( const char* basename="sigmaG1_vs_lapSigmaG1_for_root", 
