...
//   Double_t t_max = t0 + amax/v0;
   ...
   if ( t>=t0 && t<=(t_max) ){
      ...
      Double_t sqrt_arg = amax*v0*dt - (v0*dt)*(v0*dt);
      printf("DEBUG : t = %.4f vs. [%.4f,%.4f] -> sqrt_arg = %.8f\n",t,t0,t_max,sqrt_arg);
      if( sqrt_arg >= 0 ){
...
//    pGraph->GetXaxis()->SetLimits(0,1);
   ...
   if( fit_func_name && strlen(fit_func_name) ){
      if( strcmp( fit_func_name, "dm_vs_mjd" )==0 ){
         ...
         line = new TF1("fit_func",dm_vs_mjd,fit_min_x,fit_max_x,5);
         line_draw = new TF1("fit_func2",dm_vs_mjd,minX,maxX,5);
         local_func=1;

         par[0] = 1.00; // ne
         par[1] = 0.0001; // amax
         par[2] = 1; // transverse velocity of Crab 
         par[3] = 0.005; // dm0
         par[4] = 30.00; // t0
         ...
         line->SetParName(1,"amax");
         line->SetParName(2,"#alpha");
         line->SetParName(3,"DM0");
         line->SetParName(4,"t0");
...
void fit_dm_variations( const char* basename="sigmaG1_vs_lapSigmaG1_for_root", 
