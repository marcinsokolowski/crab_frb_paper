...
//         line->Draw("same");
         ...
         printf("Completness threshold for SNR = 8 is %.8f Jy\n",line_draw->Eval(8));
         printf("Completness threshold for SNR = 10 is %.8f Jy\n",line_draw->Eval(10));
      }
...
void plot_flux_vs_snr( const char* basename="sigmaG1_vs_lapSigmaG1_for_root", 
               const char* fit_func_name="line", double min_y=-10000, 
               double max_y=-10000 , int bLog=0,
      const char* szDescX="SNR",const char* szDescY="Flux density [Jy]", const char* szTitle=NULL,
      double fit_min_x=-100000, double fit_max_x=-100000,
