...
Double_t Gaussian( Double_t* x, Double_t* y )
{
   ...
   double gaussian = norm*exp(-0.5*(t - t_peak)*(t - t_peak)/(2.00*sigma*sigma) ) + offset;
...
void histo_time_between_pulses_log_normal( const char* fname, 
                int column=0,
...
// return; 
   ...
   if( dofit ){
      ...
      pFitFunc->SetParName(0,"Offset");
      pFitFunc->SetParName(1,"log10(T_{0})");
      pFitFunc->SetParName(2,"Norm");
      pFitFunc->SetParName(3,"#sigma");
