...
double count_gps_with_noise( TF1* fluence_distrib, double F_min, double fluence_bin, double sigma_n, double snr_threshold, Double_t* par )
{
   ...
   while( F < 10000 ){
      ...
      TF1* pulse_no_norm = new TF1("Pulse_with_gauss_onset_NONORM",Pulse_with_gauss_onset_NORM,-0.02,20*tau,5);
      par_local[2] = 1.00;
      pulse_no_norm->SetParameters(par_local);
      double F_test = pulse_no_norm->Integral( -0.02, 20*tau, 1e-10  )*1000.00;
      double N = F_c / F_test;
      par_local[2] = N;
      pulse_no_norm->SetParameters(par_local);
      F_test = pulse_no_norm->Integral( -0.02, 20*tau, 1e-10 )*1000.00;
...
void fluence_vs_flux_model_norm_noise_tau_dep_FLUENCE_DETECTOR( double n_tau=4.00,  // number of timesamples in the BOXCAR
                                                    double p0_mult = 1.00, 
                                                    double p1_index=-2.99970, 
                                                    double start_tau=0.0005, 
                                                    double noise_multiplier=1.00 
                                                  ) // fluence is constant in Jy*ms as in Figure 7 in the paper at rate 1-GP/hour :
{
   ...
   TF1* fluence_distrib_full = new TF1("Fluence_distrib_powerlaw0",Fluence_distrib_powerlaw,90,5000,2);
   Double_t par_distr[2];
   par_distr[0] = 4.58470e+01; // was 18.00;
   par_distr[1] = -3.33436; // was -3.132;
   fluence_distrib_full->SetParameters(par_distr); 
   fluence_distrib_full->Draw();   
   c1->Update();
...
//   TF1* pulse = new TF1("Pulse_with_gauss_onset",Pulse_with_gauss_onset,-0.02,0.02,5);
   ...
   char szOutFile[128];
   sprintf(szOutFile,"FLUENCE_DETECTOR.txt",int(F_1hour),sigma_n);
...
//   tau = 0.002;
   ...
   int nt = 100;
   int n_simul = 10;
   Double_t* simul_t_arr = new Double_t[nt];
   Double_t* simul_f_arr = new Double_t[nt];
   // calculate noise in fluence 
//   double n_tau=4.00; // 4.00 is my template does not really change the final fluence threshold that much 
/*   tau = start_tau;
   while( tau <= 0.010 ){
      double boxcar = n_tau*tau;
      double simul_mean_fluence_rms = 0;
      int simul_mean_fluence_count = 0;
      for(int s=0;s<n_simul;s++){ 
         double simul_fluence = 0.00, simul_fluence2 = 0.00;
         int simul_fluence_count=0;
         for(int t=0;t<nt;t++){
            simul_t_arr[t] = t*gTimeResolution;
            simul_f_arr[t] = gRandom->Gaus( 0.00, sigma_n );            
 
            if( simul_t_arr[t] < boxcar ){
               simul_fluence += simul_f_arr[t];
               simul_fluence2 += (simul_f_arr[t])*(simul_f_arr[t]);
               simul_fluence_count++;
            }
         }

         double simul_fluence_mean = simul_fluence / simul_fluence_count;
         double simul_fluence_rms = sqrt( simul_fluence2/simul_fluence_count - simul_fluence_mean*simul_fluence_mean ); 

         printf("\tDEBUG : simul(%d) fluence_rms = %.6f [Jy ms] based on %d time bins\n",s,simul_fluence_rms,simul_fluence_count);

         simul_mean_fluence_rms += simul_fluence_rms;
         simul_mean_fluence_count++;         
      }
      simul_mean_fluence_rms = simul_mean_fluence_rms / simul_mean_fluence_count;
      printf("Tau = %.3f [ms] : FLUENCE_RMS = %.6f [Jy ms] -> Threshold = %.6f [Jy ms]\n",tau*1000.00,simul_mean_fluence_rms,5.00*simul_mean_fluence_rms);
...
*/
   ...
   while( tau <= 0.010 ){
      ...
      // see: Figure 1 in /home/msok/Desktop/EDA2/papers/2024/EDA2_FRBs/20250503_modelling_number_of_GPs_SIMPLIFIED_TauDepFluenceDistrb.odt
      // and Figure 7 in 20250502_refitting_fluence_distributions.odt
      TF1* fluence_distrib = new TF1("Fluence_distrib_powerlaw0",Fluence_distrib_powerlaw,90,5000,2);
      double tau_ms = tau*1000.00;
      par_distr[0] = 131.6 - 30.13*tau_ms; // 4.58470e+01; // was 18.00;
//      par_distr[0] = 200 - 30*tau_ms; // ~140 
//      par_distr[0] = 239.371/tau_ms -45.4504; //~80
//      par_distr[0] = 431.382/(tau_ms-(-0.889364))-75.1216;
      par_distr[1] = -3.33436; // was -3.132;
      fluence_distrib->SetParameters(par_distr);
