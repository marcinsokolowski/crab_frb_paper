...
listfile=process.list
if [[ -n "$1" && "$1" != "-" ]]; then
   listfile="$1"
fi

SNR_THRESHOLD=5 # calibrate all pulses 
if [[ -n "$2" && "$2" != "-" ]]; then
   SNR_THRESHOLD=$2
fi
...
do
   ...
   if [[ -s $datfile ]]; then     
      ...
      # use maximum SNR 
      echo "~/github/crab_frb_paper/scripts/calib/calibrate_pulses.sh ${MEAN_SEFD} ${SNR_THRESHOLD} - \"${root_options}\" # ${SNR_THRESHOLD}"
      ~/github/crab_frb_paper/scripts/calib/calibrate_pulses.sh ${MEAN_SEFD} ${SNR_THRESHOLD} - "${root_options}" # ${SNR_THRESHOLD}
