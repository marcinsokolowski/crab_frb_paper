...
{
    Double_t time_lag=x[0];

    return y[0]*TMath::Power( time_lag , y[1] );
}
...
//    pGraph->GetXaxis()->SetLimits(0,1);
   ...
   if( fit_func_name && strlen(fit_func_name) ){
      ...
      if( strstr(fit_func_name,"line") || fit_func_name[0]=='l' || fit_func_name[0]=='L'
          || fit_func_name[0]=='h' || fit_func_name[0]=='H' || fit_func_name[0]=='p' || strstr(fit_func_name,"power_law") ){
...
void plot_structure_function( const char* basename="sigmaG1_vs_lapSigmaG1_for_root", 
               const char* fit_func_name="power_law", double min_y=-10000, 
               double max_y=-10000 , int bLog=0,
      const char* szDescX="Time lag [days]",const char* szDescY="Structure function", const char* szTitle=NULL,
      double fit_min_x=8.00, double fit_max_x=26.00,
