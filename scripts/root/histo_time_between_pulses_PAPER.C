...
double gFlux0 = 10.00; // better to have normalisation at time interval between pulses = 10 seconds
...
void histo_time_between_pulses_PAPER( const char* fname, int column=0,
...
//      parerrors=histo->GetFunction("power_law_distrib")->GetParErrors();
      ...
      histo->GetFunction("power_law_distrib")->SetParName(0,"N");
      histo->GetFunction("power_law_distrib")->SetParName(1,"#beta");
      ...
      sprintf(szDesc,"N(#Delta t) = N_{0} #left(#frac{#Delta t}{10s}#right)^{#beta}",base);
      lat.DrawLatex( 20.0,10.0,szDesc);
