...
int gLog=0;
int gVerb=0;
double gFluxDensityThreshold=500;
...
//         line->Draw("same");
         ...
         printf("Threshold in fluence = %.8f [Jy] corresponding to flux threshold = %.8f [Jy]\n",line_draw->Eval(gFluxDensityThreshold),gFluxDensityThreshold);
      }
...
void plot_fluence_vs_flux( const char* basename="sigmaG1_vs_lapSigmaG1_for_root", double flux_density_threshold=500,
               const char* fit_func_name="line", double min_y=-10000, 
               double max_y=-10000 , int bLog=0,
      const char* szDescX="Flux density [Jy]",const char* szDescY="Fluence [Jy ms]", const char* szTitle=NULL,
      double fit_min_x=-100000, double fit_max_x=-100000,
      int x_col=0, int y_col=1, const char* outpngfile=NULL )
{
   ...
   gLog = bLog;
   gFluxDensityThreshold = flux_density_threshold;
