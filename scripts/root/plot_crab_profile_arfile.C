...
/*         if( gNormaliseInputData ){
      ...
      if( strcmp( fit_func_name, "pulse_gauss" )==0 ){
         ...
         par[1] = 0.8; // t_peak  0.53 for the MWA data
         par[2] = 1; // peak flux 
         par[3] = 0.1; // sigma 
...
void normalise_x( Double_t* x_values, int cnt )
{
   ...
   printf("X axis offset    = %.8f\n",gXaxisOffset);
   printf("X scaling factor = %.8f\n",gXaxisNormFactor);
}
...
void plot_crab_profile_arfile( const char* basename="sigmaG1_vs_lapSigmaG1_for_root", int bNormaliseInputData=2, bool bShowOriginalDataWithFit=false,
                       const char* fit_func_name="pulse_gauss", // pulse, pulse_gauss, pulse_gauss_only
                       double noise_start=0, double noise_end=0.5, 
