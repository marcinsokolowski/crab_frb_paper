...
// norm at 55.7 
void plot_pulse_time_vs_expected( const char* basename="sigmaG1_vs_lapSigmaG1_for_root", 
                          const char* basename2="sigmaG1_vs_lapSigmaG1_for_root",         
...
// mg->SetMinimum(1);  
   ...
   TGraph* pGraph1 = DrawGraph( x_value1, y_value1, lq1, 1, NULL, kBlack, "PA", 1, kFullCircle,
...
//   pad2->cd();
   ...
   // NORMALIZATION : 
//   double power1 = find_freq(x_value1, y_value1, lq1, norm_freq );
//   double power2 = find_freq(x_value2, y_value2, lq2, norm_freq );
//   double ratio = (power1/power2)*1;
//   printf("Normalizing second file to power around %.2f [MHz], %e / %e = %.8f\n",norm_freq,power1,power2,ratio);
//   for(int i=0;i<lq2;i++){
//      y_value2[i] = y_value2[i] * ratio;
//   }
...
/*   for(int i=0;i<lq2;i++){
      TLine *line = new TLine(x_value2[i],0,x_value2[i],20);
   ...
   if( cand_file ){
      ...
      cand_count = ReadResultsFile( cand_file, x_value_cand, y_value_cand, -1, -1, 2, 0 );
