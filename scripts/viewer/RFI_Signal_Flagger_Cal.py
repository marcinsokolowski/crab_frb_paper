...
timeres_sec = 0.00096768 # 1.08*64*14/1000000.00 seconds 

def parse_options():
   usage="Usage: %prog [options]\n"
   usage+='\tDe-dispersion with a specified DM\n'
...
#   parser.add_option('-f','--fits','--image','--fitsfile',dest="fitsfile",default="avg_I_without.fits", help="Fits image file to calibrate with GLEAM",metavar="STRING")
   parser.add_option('-c','--calibrate','--cal',dest="calibrate",action="store_true",default=False, help="Calibrate flux density in Jy using SEFD [default %]")
   parser.add_option('--sefd',dest="sefd",default=7911.00,help="SEFD of the telescope in Jy [default %default Jy]",type="float")
...
def plotting( time_xaxis, total_power_vs_time, pixel_data, new_pixel_data, time_xaxis_signal, total_power_vs_time_signal, 
              rfi_threshold_median_above,rfi_threshold_median_below, median, median_signal, signal_threshold_median_above, signal_threshold_median_below,
              calibrate=True, bw_hz=31250000, npol=2, sefd_jy=7911 # , timeres_sec=0.00096768
            ):
    ...
    print("SNR_uncal = %.2f" % (snr))
    
    # tt=("SNR = %.2f" % (snr))
    # plt.text( arg_max_value, max_value*0.75, tt )        
    
    if calibrate :
       sigma_sim=sefd_jy/math.sqrt(npol*bw_hz*timeres_sec)
       for i in range(0,len(total_power_vs_time_signal)):
          total_power_vs_time_signal[i] = (total_power_vs_time_signal[i] - median)/IQR_rms
          total_power_vs_time_signal[i] = total_power_vs_time_signal[i]*sigma_sim
          
    # calculate SNR of maximum peak in de-dispersed total power using calibrated values:
    sorted_values=np.array(total_power_vs_time_signal)
    max_value=sorted_values.max()
    arg_max_value=sorted_values.argmax()
    sorted_values.sort()
    ...
    print("Dedispersed IQR_rms = %.4f, median = %.4f , max = $%.4f" % (IQR_rms,median,max_value))
    snr = (max_value - median)/IQR_rms
    print("SNR_cal = %.2f" % (snr))

    # fluence :
    peak_start = arg_max_value - 20 # 10 time samples before the peak 
    sum = 0
    if peak_start < 0 :
       peak_start = 0
    added = True 
    pos = peak_start
    while added and pos < len(total_power_vs_time_signal) :
       if (total_power_vs_time_signal[pos] - median) > 2*IQR_rms :
          delta = (total_power_vs_time_signal[pos] - median)
          sum += delta
          print("DEBUG : added %.6f [Jy] (snr=%.3f) at %.3f [ms] (bin %d vs. max bin = %d)" % (delta,delta/IQR_rms,time_xaxis_signal[pos],pos,arg_max_value))
       else :
          # if we are past the peak and signal below 1sigma above median finish summing
          if pos > arg_max_value : 
             added = False
...
def is_there_RFI(fits_file):
    ...
    plotting(time_xaxis, total_power_vs_time, pixel_data, new_pixel_data, time_xaxis_signal, total_power_vs_time_signal, rfi_threshold_median_above,rfi_threshold_median_below, median, median_signal, signal_threshold_median_above, signal_threshold_median_below,
             calibrate=options.calibrate, sefd_jy=options.sefd )
    return RFI_or_not
