...
void fluence_vs_flux_model_norm_noise_tau_dep_FINAL_PAPER( double p0_mult = 1.00, double p1_index=-2.99970, double start_tau=0.0005, double noise_multiplier=1.00 ) // fluence is constant in Jy*ms as in Figure 7 in the paper at rate 1-GP/hour :
{
...
//   TF1* pulse = new TF1("Pulse_with_gauss_onset",Pulse_with_gauss_onset,-0.02,0.02,5);
   ...
   char szOutFile[128];
   sprintf(szOutFile,"peakflux_vs_tau_%dJyms_noise%.3fJy_TAUDEP.txt",int(F_1hour),sigma_n);
...
// TEMPLATE:      par_distr[0] = 131.6/0.7 - 30.13*tau_ms; // 4.58470e+01; // was 18.00;
//      par_distr[0] = 200 - 30*tau_ms; // ~140 
      par_distr[0] = 239.371/tau_ms -45.4504; // Figure 7 in /home/msok/Desktop/SKA/papers/2024/EDA2_FRBs/20250502_refitting_fluence_distributions.odt
...
//      c2->cd();
      ...
      double alpha = 0.62; // effectively PRESTO threshold is slightly lower than the theoretical 5sigma ? because it's a fluence detector by using BOXCAR ?
...
//      F_min = 5*sigma_n;
//      printf("DEBUG : F_min_new = %.8f [Jy ms]\n",F_min);
      
      double N_gp = count_gps( fluence_distrib, F_min, fluence_bin ); // 0.75 works ok
