...
void UniversalPowerLawDistrib( const char* fname, double TotalTimeInHours=0.998768602,
                double low=0, double up=1000, int bin_no=100, // SNR 5 - 110 , Flux : 150 - 1000
                double fit_min_x=5, double fit_max_x=500, 
                const char* szTitleX="Signal-to-noise ratio (SNR)", const char* szTitleY="Rate of events / hour",
                int column=0,
...
//   if( DoBorder <= 0 )
   double start_x = 1;
   double   border=0;
   printf("DEBUG : histo bin_no = %d, range = %.6f - %.6f\n",bin_no,low-border,up+border);
