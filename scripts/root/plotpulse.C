...
void calc_mean_rms( Double_t* x_value1, Double_t* y_value1, int lq1, double bkg_start, double bkg_end, double& mean, double& rms )
{
   double sum=0,sum2=0;
   int count=0;
   ...
   for(int i=0;i<lq1;i++){
      ...
      if( x>=bkg_start && x<=bkg_end ){
         sum += y_value1[i];
         sum2 += (y_value1[i])*(y_value1[i]);
         printf("DEBUG : %.8f %.8f -> sum = %.8f\n",x,y_value1[i],sum);
         count++;
      }
   }
   ...
   printf("%.8f - %.8f : mean = %.8f , rms = %.8f\n", bkg_start,bkg_end,mean,rms); 
}

void plotpulse( const char* basename="sigmaG1_vs_lapSigmaG1_for_root", double bkg_start=-1e20, double bkg_end=1e20,
               const char* fit_func_name=NULL, double min_y=-10000, 
               double max_y=-10000 , int bLog=0,
      const char* szDescX=NULL,const char* szDescY=NULL, const char* szTitle=NULL,
      double fit_min_x=-100000, double fit_max_x=-100000,
      int x_col=0, int y_col=1, const char* outpngfile=NULL )
{
   ...
   double mean, rms;
   calc_mean_rms( x_value1, y_value1, lq1, bkg_start, bkg_end, mean, rms );
