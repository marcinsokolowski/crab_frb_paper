...
def parse_options(idx):
   ...
   parser.add_option('--presto_format','--presto_input','--presto',action="store_true",dest="presto_input",default=False, help="PRESTO format of input files [default %default]")
...
def read_file(file,verb=False,frbsearch_input=False,presto_input=False) :
   cand_list = []
   ...
   for line in data : 
      ...
      if frbsearch_input : # input as from frb_search package 
         ...
         if presto_input : 
            ...
            #  102.00    5.61    674.942285     697485      30
            try : 
               dm  = float( words[0+0] )
               snr = float( words[1+0] )
               t   = float( words[2+0] )
               sample = float( words[3+0] )
               downfact = float( words[4+0] )
            except :
               print("ERROR while parsing line |%s| -> |%s| |%s| |%s| |%s| |%s|" % (line,words[0+0],words[1+0],words[2+0],words[3+0],words[4+0]))
               sys.exit(-1)
         else :
            # normal FREDDA output :
