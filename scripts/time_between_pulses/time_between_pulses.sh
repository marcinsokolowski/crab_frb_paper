...
do
   ...
   pwd
   awk '{if($1!="#"){print $5;}}' presto.cand_merged |sort -n | awk -v prev_time=0 '{if(NR>1){print $1-prev_time;}prev_time=$1;}' > ${ut}_time_diff.txt
   ...
   if [[ $good_time -ge 3000 ]]; then
      ...
      cat ${ut}_time_diff.txt >> ${outdir}/all_good_time_diff.txt
      cat time_between_pulses_fit_results.txt | awk -v good_time=${good_time} -v uxtime=${uxtime} '{print uxtime" "$0" "good_time;}' >> ${outdir}/all_good_time_between_pulses_fit_results.txt
   fi
   
   # all pulses go here (including good and bad nights)
   cat ${ut}_time_diff.txt >> ${outdir}/all_time_diff.txt
   cat time_between_pulses_fit_results.txt | awk -v good_time=${good_time} -v uxtime=${uxtime} '{print uxtime" "$0" "good_time;}' >> ${outdir}/all_time_between_pulses_fit_results.txt            
...
cp ~/github/crab_frb_paper/scripts/root/histo_time_between_pulses.C .
mkdir -p images   
root -l "histo_time_between_pulses.C(\"all_time_diff.txt\")"
root -l "histo_time_between_pulses.C(\"all_good_time_diff.txt\")"

cat all_good_time_between_pulses_fit_results.txt | awk '{print $1" "43200" "$3" "$5;}' > good_time_powerlaw_index_vs_time.txt
root -l "plot_timeplidx_vs_time_error.C(\"good_time_powerlaw_index_vs_time.txt\")"

cat all_good_time_between_pulses_fit_results.txt | awk '{print $1" "43200" "$2" "$4;}' > good_time_powerlaw_norm_vs_time.txt
root -l "plot_timeplidx_vs_time_error.C(\"good_time_powerlaw_norm_vs_time.txt\",0,50,\"Power law norm\")"
