...
TGraphErrors* DrawGraph( Double_t* x_values, Double_t* y_values, int numVal, 
         ...
         double fit_min_x=-100000, double fit_max_x=-100000,
                        Double_t* y_values_err=NULL, const char* szOPT="AP", int MarkerType = 20, int ColorNum = kBlack, int marker_size=2 )
{
   ...
   if( gVerb ){
        ...
        if( y_values_err ){
           pGraph->SetPointError( i, (0.5/24.00), y_values_err[i] ); // 1/2 hour observation error.
        } 
    }
...
*/
    ...
    pGraph->SetMarkerSize(marker_size);
...
int merge( Double_t* x_values, Double_t* y_values, Double_t* x_values_err, Double_t* y_values_err, int& all )
{
   if( gMergeSeconds > 0 ){
      ...
      Double_t* merged_y_err = new Double_t[all];
      int merged = 0;
      ...
      for(int i=0;i<all;i++){
         double uxtime = x_values[i];

         if( uxtime > 0 ){ // <0 indicates that it was already used for earlier merge
            printf("Merging uxtime = %.8f\n",uxtime);
            double sum_x = x_values[i], sum_y = y_values[i], err2_y = y_values_err[i]*y_values_err[i];
            int merged_count = 1;
            for(int j=(i+1);j<all;j++){
               if( x_values[j] > 0 and fabs(x_values[j]-x_values[i])<=gMergeSeconds )
               {
                  sum_x += x_values[j];
                  sum_y += y_values[j];
                  err2_y += y_values_err[j]*y_values_err[j];
                  merged_count++;
                  printf("\tadded : %.8f , %.8f +/- %.8f\n",x_values[j],y_values[j],y_values_err[j]);

                  x_values[j] = -1;
               }
            }

            merged_x[merged] = sum_x/merged_count;
            merged_y[merged] = sum_y/merged_count;
            merged_y_err[merged] = sqrt(err2_y)/merged_count;
            printf("\t%d points merged to %.8f , %.8f +/- %.8f\n",merged_count,merged_x[merged],merged_y[merged],merged_y_err[merged]);
            merged++;
         }
      }
...
void parse_legend(const char* basename, char* szLegend)
{
   if(strstr(basename,"DM_PEAK_pulse_vs_ux")){
      strcpy(szLegend,"DM by maximising GP peak flux (this work)");
   }
   if(strstr(basename,"DM_vs_UX_Pratik")){
      strcpy(szLegend,"DM timing measurements (this work)");
   }
   if(strstr(basename,"jodrell")){
      strcpy(szLegend,"DM Jodrell-Bank measurements");
   }
}

void plot_dm_vs_time_merger_PAPER( const char* basename="sigmaG1_vs_lapSigmaG1_for_root", 
      ...
      double min_uxtime=-10000, double max_uxtime=-10000, const char* outdir="images/" )
{
   ...
   lq1_err = merge( x_value1, y_value1, x_value1_err, y_value1_err, lq1 );
...
//   printf("DEBUG : %s\n",da.AsString());
   ...
   TGraphErrors* pGraph1 = DrawGraph( x_value1, y_value1, lq1, 1, NULL, 
            ...
            fit_max_x, y_value1_err, "AP", 20, kRed, 2 );
   ...
   legend->AddEntry(pGraph1,szLegend,"P");
...
//      lq2_err = merge( x_value2, y_value2, x_value2_err, y_value2_err, lq2 );

      TGraphErrors* pGraph2 = DrawGraph( x_value2, y_value2, lq2, 1, NULL, fit_func_name, min_y, max_y, szTitle, basename, bLog, szDescX, szDescY, fit_min_x, fit_max_x, y_value2_err, "P,L,same", 29, kBlack, 3  );

//      legend->AddEntry(pGraph2,"Coherent (.ar files)","P");     
      parse_legend(basename2,szLegend);
      legend->AddEntry(pGraph2, szLegend, "PL");
   }
...
//      legend->AddEntry(pGraph3,"Coherent (.ar files)","P");     
      ...
      legend->AddEntry(pGraph3, szLegend, "P");
   }
